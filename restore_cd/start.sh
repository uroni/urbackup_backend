...
./urbackup_client --no-server --plugin ./libfsimageplugin.so --plugin ./libfileservplugin.so --plugin ./liburbackupclient.so --restore_mode true --logfile restore_mode.txt --loglevel debug > /dev/null 2>&1 &disown
sleep 1
