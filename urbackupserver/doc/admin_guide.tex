...
a 64-bit operating system and at least Windows Vista/2008.
   ...
   \begin{itemize}
     ...
     \item If you are using Windows Server 2008(R2) (or the equivalent Desktop versions) you should consider consider applying hotfix \url{https://support.microsoft.com/en-us/kb/967351} and then formatting the backup storage volume with \begin{verbatim}
...
Currently there are to machisms to estimate how much data the client can transfer per month with its current Internet connection. If the client is running on Windows 10, the client is connected via Wifi and the Wifi connection is set to be metered, the data usage limit is estimated at 1GB per month.
