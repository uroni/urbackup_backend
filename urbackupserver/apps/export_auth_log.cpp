...
int export_auth_log()
{
	...
	if(!out.is_open())
	{
		Server->Log("Error opening \""+Server->getServerWorkingDir()+os_file_sep()+"urbackup/auth_log.csv\" for writing", LL_ERROR);
		return 1;
	}
	...
	out.close();
	Server->Log("Auth log has been written to \""+Server->getServerWorkingDir()+os_file_sep()+"urbackup/auth_log.csv\"", LL_INFO);
