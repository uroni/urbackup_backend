...
DLLEXPORT void LoadActions(IServer* pServer)
{
	...
	IFsFile* alloc_test = Server->openFile("D:\\tmp\\alloc_test.dat", MODE_RW_CREATE_RESTORE);

	alloc_test->Resize(1 * 1024 * 1024 * 1024, false);

	char ch = 1;
	alloc_test->Write(500 * 1024 * 1024, &ch, 1);
	alloc_test->Sync();

	bool more_data;
	std::vector<IFsFile::SFileExtent> exts = alloc_test->getFileExtents(0, 4096, more_data);

	FILE_STANDARD_INFO info;
	GetFileInformationByHandleEx(alloc_test->getOsHandle(), FileStandardInfo, &info, sizeof(info));

	_FILE_STREAM_INFO* info2 = (_FILE_STREAM_INFO*)_aligned_malloc(1000, 32);

	GetFileInformationByHandleEx(alloc_test->getOsHandle(), FileStreamInfo, info2, 1000);

	BY_HANDLE_FILE_INFORMATION hfi;
	GetFileInformationByHandle(alloc_test->getOsHandle(), &hfi);

	IFsFile* dev = Server->openFile("\\??\\D:", MODE_READ_DEVICE);
