...
std::vector<SFileAndHash> IndexThread::getFilesProxy(const std::string &orig_path, std::string path, const std::string& named_path, bool use_db, const std::string& fn_filter, bool use_db_hashes)
{
	...
	if (!use_db || dir_changed)
	{
		...
		fs_files = convertToFileAndHash(orig_path, named_path, os_files, fn_filter);
	...
	{	
		if( cd->getFiles(path_lower, get_db_tgroup(), fs_files) )
		{
			...
			handleSymlinks(orig_path, named_path, fs_files);
...
std::vector<SFileAndHash> IndexThread::convertToFileAndHash( const std::string& orig_dir, const std::string& named_dir, const std::vector<SFile> files, const std::string& fn_filter)
{
...
void IndexThread::handleSymlinks(const std::string& orig_dir, const std::string& named_dir, std::vector<SFileAndHash>& files)
{
	for (size_t i = 0; i < files.size(); ++i)
	{
		if (files[i].issym)
		{
			if (skipFile(orig_dir + os_file_sep() + files[i].name, named_dir + os_file_sep() + files[i].name))
			{
				continue;
			}
