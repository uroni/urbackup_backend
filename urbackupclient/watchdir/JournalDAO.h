...
public:
	...
	void addRoot(const std::wstring& root_name);
	void addFrn(const std::wstring& name, int64 pid, int64 frn, int64 rid);
	void resetRoot(int64 rid);
	CondInt64 getFrnEntryId(int64 frn, int64 rid);
	std::vector<int64> getFrnChildren(int64 pid, int64 rid);
	void delFrnEntry(int64 id);
	SNameAndPid getNameAndPid(int64 frn, int64 rid);
	...
	void updateFrnNameAndPid(const std::wstring& name, int64 pid, int64 id);
	void insertJournalData(const std::wstring& device_name, int64 journal_id, int64 usn, int64 reason, const std::wstring& filename, int64 frn, int64 parent_frn, int64 next_usn, int64 attributes);
	...
	void delFrnEntryViaFrn(int64 frn, int64 rid);
